...
if (isset($_GET['bid'])) {	
	$bid=$_GET['bid'];
	$sql1="SELECT * FROM buyer WHERE bid=$bid AND status=1";
...
if (isset($_POST['getpaid'])) {
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$currdate= date("Y-m-d"); 
          $sql = "UPDATE buyer_deposit SET amt='$amt', order_status='Success', order_id='$order_id', tracking_id='$tracking_id', pay_mode='$pay_mode', trans_date='$currdate', pay_status=1 WHERE bid=$bid";
		  ...
		  header( "Refresh:1; url=./buyer_getpaid.php?bid=$bid&alert=$alert&show=$show&error=$error", true, 303);
          }
          else{
...
<title>Add Buyer Payment Details </title>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 col-md-12">
        <h2 class="page-title remove-top-padding">Buyer Payment</h2>
        <div class="row">
         <?php
      ...
      $sql = "SELECT * FROM buyer_deposit WHERE bid=$bid AND pay_status=1";
        ...
        // output data of each row
          ...
          while($row = $result->fetch_assoc()) {
           echo"<tr>";
              ...
              echo  "<td><button type='submit' onclick='delete_buyer_deposit(".$row['bd_id'].")' class='btn btn-default btn-sm' name ='btndel' id='btndel'> <span class='glyphicon glyphicon-trash'></span> Delete</button></td>";
           echo "</tr>";
         }
          echo"</tbody>
      </table>";
        ...
        ?> 
            ...
            <div class="panel panel-info light-pink">
              <div class="panel-heading">
                <h3 class="panel-title">Get Buyer Payment</h3>
              </div>
              <div class="panel-body">
                					 					
               <form method="post" name="myforms" onsubmit="return ValidatePass()" action="./buyer_getpaid.php?bid=<?php echo $bid;?>">
			  <div class="form-group">
				<label for="sname">Buyer Name</label>
				<input type="hidden" name="bid" class="form-control" value="<?php echo $bid?>" readonly required />
				<input type="text" name="bname" class="form-control" value="<?php echo $bname?>" readonly required />
			  </div>
			  <div class="form-group">
