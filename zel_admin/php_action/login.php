...
if(isset($_SESSION['login_user_user']) && isset($_SESSION['login_pass_user']))
{
...
if(isset($_POST['submitlogin'])){
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		$sql="SELECT * FROM user WHERE mobile='$myusername' AND status=1";
		...
		if ($result->num_rows > 0){
			...
			{
				$_SESSION['login_user_user']=$mobile;
				$_SESSION['login_pass_user']=$pass;
				...
				$sql="UPDATE user SET last_login_date='$currdate', last_login_ip='$ip', latitude='$latitude', longitude='$longitude' WHERE uid=$uid";
				$conn->query($sql);
				header("location:../profile.php");
				exit();
			}
