...
/*if(isset($_POST['rsubmit'])){
	...
	if($code===$user_code){				
		...
		$sql1="SELECT * FROM seller WHERE status=1 AND mobile='$mobile'";
		//$result=mysql_query($sql); 
		$result = $conn->query($sql1);
		if ($result->num_rows > 0){
			$error=$sname ." "."User Mobile is already exist! If you forgot your password please <a href='./forgot_pass.php'>Click Here</a> !";
			$show="display:show;";
		...
		else 
		{
			if($pass===$cpass)
			{
				...
				$sql = "INSERT INTO seller (sname, mobile, pass, email, address, city, taluka, dist, pincode, latitude,longitude,regdate, status)
				VALUES ('$sname', '$mobile', '$strpass', '$email','$address', '$city', '$taluka', '$dist','$pincode','$latitude','$longitude', '$currdate', '$status')";
				// use exec() because no results are returned
				if ($conn->query($sql) === TRUE) {	
					$sid = $conn->insert_id;
					$sqlprvedu="INSERT INTO seller_deposit (sid) VALUES ($sid)";
					$conn->query($sqlprvedu);								
					//code for sms sending
					$message="Welcome ".$sname." Your Account is Successfully created. Your User Name: ".$mobile." and password: ".$pass." Thank You.";
					$mobile_number=$mobile;				  
					$message1= sms_unicode($message);
					sentsms($message1, $mobile_number);	
					$_SESSION['login_user_seller']=$mobile;
					$_SESSION['login_pass_seller']=$strpass;					
        ...
        if(is_uploaded_file($_FILES['userfile']['tmp_name'])) {    
            if( $_FILES['userfile']['size'] < $maxsize) {  
                ...
                if(strpos(finfo_file($finfo, $_FILES['userfile']['tmp_name']),"image")===0) {    
					...
					$sqlprvedu="INSERT INTO seller_deposit (sid) VALUES ($sid)";
					$conn->query($sqlprvedu);	
					$sqldocs="INSERT INTO seller_doc (imgpath, photo_date, photo_status, sid) VALUES ('$imgpath', '$currdate', 1, $sid)";
					$conn->query($sqldocs);								
					//code for sms sending
					$message="Welcome ".$sname." Your Account is Successfully created. Your User Name: ".$mobile." and password: ".$pass." Thank You.";
					$mobile_number=$mobile;				  
