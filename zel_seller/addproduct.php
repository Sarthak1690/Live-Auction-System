            ...
            if( $_FILES['userfile']['size'] < $maxsize) {  
                ...
                if(strpos(finfo_file($finfo, $_FILES['userfile']['tmp_name']),"video/mp4")===0) {    
                    ...
                    $sql = "INSERT INTO product (pname, variety, area, size, production,sugar,global_gap,farm_reg,residue_free_check,residue_result,paddress,pdesc, latitude, longitude,vdopath, regdate, status, sid) 
							VALUES ('$pname', '$variety', '$area', '$size','$production', '$sugar', '$global_gap', '$farm_reg','$residue_free_check', '$residue_result','$paddress','$pdesc', '$latitude', '$longitude', '$vdopath', '$currdate',2, $sid)";
...
<script language="javascript">
	...
	function changeresidue(obj){
    var selectedtype= (obj.options[obj.selectedIndex].value);
	if(selectedtype=="Yes")
	{
		document.getElementById("residue_result_div").style="display:show;";
	}
	else
	{
		document.getElementById("residue_result_div").style="display:none;";
		document.getElementById("residue_result").value="-";
		document.getElementById("global_gap_div").style="display:none;";
		document.getElementById("global_gap").value="-";			
	}
    }
</script>
<script type="text/javascript">
	function changeresidue_result(obj){
    var selectedtype= (obj.options[obj.selectedIndex].value);
	if(selectedtype=="Pass")
	{
		document.getElementById("global_gap_div").style="display:show;";
	}
	else
	{
		document.getElementById("global_gap_div").style="display:none;";
  ...
  <script type="text/javascript">
		function validation(){
			var prdvdo=document.forms['addproduct']['userfile'];
			var valid_Ext=["AVI","FLV","WMV","MP4","AVCHD","MOV","MKV","avi","flv","wmv","mp4","avchd","mov","mkv"];
			if(prdvdo.value!=''){
				var img_ext=prdvdo.value.substring(prdvdo.value.lastIndexOf('.')+1);
				var result=valid_Ext.includes(img_ext);
				if(result==false){
					document.getElementById('prdvdo').innerHTML =" ** Selected File is not Video";
					return false;
				}
				else{
					if(parseFloat(prdvdo.files[0].size/(1024*1024)) >= 3){
						document.getElementById('prdvdo').innerHTML ="File Size must be smaller than 3 MB. Current file size: "+ parseFloat(prdvdo.files[0].size/(1024*1024));
						return false;
					}	
				}				
			}
			else{
				document.getElementById('prdvdo').innerHTML =" ** No File Selected!";
				return false
			}
			return true;
		}
              ...
              <div class="panel-body">
				<div class="tab-content">
                  <div class="row well tab-pane fade in active" role="tabpanel" id="personal">
                    <div class="col-lg-12">
					<form enctype="multipart/form-data" data-toggle="validator" onsubmit="return validation()" method="post" name="addproduct" id="addproduct" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                      <div class="profile-subhead">
						...
						<div class="row">
                          <div class="col-md-6">
                            <div class="form-group">
                              <label>Farm Area (In Hectare)</label>
						...
						<div class="row">						                         
						  <div class="col-md-4">
                            <div class="form-group">
                              ...
                              <div class="select-arrowreg">
                                <select id="residue_free_check" name="residue_free_check" class="form-control" onChange="changeresidue(this)" required>
								  <option value="">Select</option>
                                  <option  value="Yes">Yes</option>
                                  <option  value="No">No</option>
                                </select>
						  ...
						  <div class="col-md-4"  id="residue_result_div" style="display:none;">
                            <div class="form-group">
                              <label>Residue Free Checkup Result?</label>
                              <div class="select-arrowreg">
                                <select name="residue_result" id="residue_result" class="form-control" onChange="changeresidue_result(this)" required>
                          ...
                          <div class="col-md-4" id="global_gap_div" style="display:none;">
                            <div class="form-group">
                              <label>Global Gap </label>
                              <div class="select-arrowreg">
                                <select name="global_gap" id="global_gap" class="form-control" required>
								  <option value="None">Select</option>
                                  <option  value="Yes">Yes</option>
                                  <option  value="No">No</option>
                                </select>
						...
						<div class="row">
                          <div class="col-md-6">
                            ...
                            <span id="prdimg" class="text-danger font-weight-bold"> </span>
							</div>
                          </div>
						  <div class="col-md-6">
                            <div class="form-group">
                            ...
                            <span id="prdvdo" class="text-danger font-weight-bold"> </span>
							</div>
                          </div>
                        </div>
						 <div class="row">
