...
if(isset($_POST['btnpayfeesonline'])){
		$amt = test_input($_POST["admission_fees"]);
		$order_id = test_input($_POST["order"]);
		$order_status="Success";
		$tracking_id="123456789";
		$payment_mode=test_input($_POST["payment_mode"]);
		$currdate=date("Y-m-d");
		$sql = "UPDATE seller_deposit SET amt='$amt', order_status='$order_status', order_id='$order_id', tracking_id='$tracking_id', pay_mode='$payment_mode', trans_date='$currdate', pay_status=1 WHERE sid=$sid";
		if ($conn->query($sql) === TRUE) {
			$sql = "UPDATE seller SET paystatus=1 WHERE sid=$sid";
			if ($conn->query($sql) === TRUE) {
				$error="Thank you for Payment. Your credit/debit card has been charged and your transaction is successful. We will be Confirm your order to you soon.";
				$show="display:show;";
				$alert="alert alert-success";
				header("Location:./orderstatus.php?error=$error&show=$show&alert=$alert&order_status=$order_status");
				exit();
			}
		}
		else{
		...
		echo "<br>Thank you for interest with us. Your credit card has been charged and your transaction is successful. We will be shipping your order to you soon.";
...
?>
 ...
 <div align="center" style="padding:1px; margin-bottom:5px;<?php echo $showalert; ?>"class="alert alert-warning" role="alert">You must have to make payment before adding the products for online auction.</div>
              ...
              <div class="panel-body">
				<div class="profile-subhead">
                        <h3 class="triangle well">Pay Auction Fees Online</h3>
							<form method="POST" name="customerData" action="./pay_fees_online.php">
							<input type="hidden" name="tid" id="tid" readonly  required/>
							<input type="hidden" name="merchant_id" value="355931" required/>
							<input type="hidden" name="order_id" value="<?php echo $sid."_".date("Ymdhis");?>" required/>
							<input type="hidden" name="amount" value="<?php echo round(5000,2);?>" required/>
							<input type="hidden" name="currency" value="INR" required/>
							<input type="hidden" name="redirect_url" value="https://abhinavgrapes.org/zel_seller/ccavenue/ccavResponseHandler.php" required/>
							<input type="hidden" name="cancel_url" value="https://abhinavgrapes.org/zel_seller/ccavenue/ccavResponseHandler.php" required/>
							<input type="hidden" name="language" value="EN" required/>
							<input type="hidden" name="billing_name" value="<?php echo $sname;?>" required/>
							...
							<input type="hidden" name="merchant_param1" value="<?php echo $sid;?>" required />
						<div class="row">
                          <div class="col-md-6">
                            <div class="form-group">
                              <label>Selected Payment Name</label>
                              <input type="text" class="form-control" name="pay_name" value="Seller Deposit" readonly  required >
                            </div>
                          </div>
                          <div class="col-md-6">
                            <div class="form-group">
						...
						<div class="row">
                          <div class="col-md-6">
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control" name="order" value="<?php echo $sid."_".date("Ymdhis");?>" readonly  required >
                            </div>
                          </div>
                          <div class="col-md-6">
                            <div class="form-group">
...
<div  id="receipt"  style="border:1px solid #ccc; padding:10px; height:100%; width:590pt; display:none;">
        ...
        <div style="text-align:center; border:0px solid #ccc; padding-top:20px;  float:center; width:100%;">
         ...
         <h4 align="center"> Seller Payment Receipt</h4><hr/>
          ...
          <tbody>
		  	<tr>
              <td style='text-align:left;  padding: 3px; border-bottom: 1px solid;'>Seller Id</td>
              <td style='text-align:left;  padding: 3px; border-bottom: 1px solid;'><span>:</span> <?php echo $sid; ?></td>
              <td style='text-align:left;  padding: 3px; border-bottom: 1px solid;'>Seller Name</td>
              <td style='text-align:left;  padding: 3px; border-bottom: 1px solid;'><span>:</span> <?php echo $sname; ?></td>
			</tr>
			<tr>
              <td style='text-align:left;  padding: 3px; border-bottom: 1px solid;'>Payment Id</td>
              <td style='text-align:left;  padding: 3px; border-bottom: 1px solid;'><span>:</span> <?php echo $sd_id; ?></td>
