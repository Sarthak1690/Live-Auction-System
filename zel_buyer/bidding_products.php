...
if (isset($_POST['submitbid']))
{
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$curr_base_rate = test_input($_POST["base_rate"]);		
		$bid_amt = test_input($_POST["bid_amt"]);		
		...
		$bid_amt=(double)$bid_amt;
		$highest_amount=(double)$highest_amount;
		$curr_base_rate=(double)$curr_base_rate;
		if($bid_amt <= $highest_amount || $bid_amt <= $curr_base_rate){
			$error="Your bidding amount must be greater than current base rate OR biiding amount!";
			$show="display:show;";
			$alert="alert alert-danger";
		}
		else{
			$sql = "INSERT INTO bidding (bid_amt, pid, bid, bid_date, bid_status)
			VALUES('$bid_amt', $pid, $bid, '$currdate', 1)";
			if ($conn->query($sql) === TRUE) {
				$error="Bidding amount submitted successfully!";
				$show="display:show;";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12 col-md-12">
        <h2 class="page-title remove-top-padding">Bidding Products</h2>
        <div class="row">
                  ...
                  <div class='profile-search-list-box'>
                    <div class='row'>
					  ...
					  <br/>";
						...
						$sqlmax="SELECT MAX(bid_amt) AS buyer_highest_amount FROM bidding WHERE pid=".$row['pid']." AND bid=$bid AND bid_status=1";
						$resultmax = $conn->query($sqlmax);
						$buyer_highest_amount=0;
						if ($resultmax->num_rows > 0){
							while($rowmax = $resultmax->fetch_assoc()) {
							$buyer_highest_amount = $rowmax['buyer_highest_amount'];
							}
						}
					  echo"<button class='btn btn-success'>Highest Bid Amount: <b>Rs. ".$highest_amount."</b></button><hr/><br/>
					  <p style='color:green'>Your Highest Bid Amount: <b>Rs. ".$buyer_highest_amount."</b></p>
							<span id='bid_amtalert' class='text-danger font-weight-bold'> </span>					  
						  <div class='row' style='background-color:#80e680;padding:3px;'>
						  <form class='form-inline' onsubmit='return validation()' id='subbid' name='subbid' method='post' action='$_SERVER[REQUEST_URI]'>
							  <div class='form-group'>
							  <input class='form-control' type='hidden' id='pid' name='pid' value=".$row['pid']." readonly>
							  <input class='form-control' type='hidden' id='base_rate' name='base_rate' value=".$row['base_rate']." readonly>
							  <input class='form-control' placeholder='Enter Bid Amount' type='text' id='bid_amt' name='bid_amt' required/>
							  
							  </div>
							  <div class='form-group'>
							  <button type='submit' name='submitbid' id='submitbid'  class='btn btn-default pink2-btn btn-on'><i class='fa fa-rupee'></i>Submit Bid</button> 
							  </div>
						  </form>
						  </div><hr/>
					  </div>
