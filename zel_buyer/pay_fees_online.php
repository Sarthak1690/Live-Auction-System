...
if(isset($_POST['btnpayfeesonline'])){
		...
		$currdate=date("Y-m-d");
		$sql = "UPDATE buyer_deposit SET amt='$amt', order_status='$order_status', order_id='$order_id', tracking_id='$tracking_id', pay_mode='Online', trans_date='$currdate', pay_status=1 WHERE bid=$bid";
		if ($conn->query($sql) === TRUE) {
			$sql = "UPDATE buyer SET paystatus=1 WHERE bid=$bid";
			if ($conn->query($sql) === TRUE) {
...
<div class="profile-subhead">
                        <h3 class="triangle well">Pay Auction Fees Online</h3>
							...
							<input type="hidden" name="redirect_url" value="https://abhinavgrapes.org/zel_buyer/ccavenue/ccavResponseHandler.php" required/>
							<input type="hidden" name="cancel_url" value="https://abhinavgrapes.org/zel_buyer/ccavenue/ccavResponseHandler.php" required/>
